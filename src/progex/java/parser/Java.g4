...
statement
    ...
    |   'synchronized' parExpression block										# synchBlockStatement
...
expression
    :   primary															# exprPrimary
    |   expression '.' Identifier										# exprDotID
    |   expression '.' 'this'											# exprDotThis
    |   expression '.' 'new' nonWildcardTypeArguments? innerCreator		# exprDotNewInnerCreator
    |   expression '.' 'super' superSuffix								# exprDotSuper
    |   expression '.' explicitGenericInvocation						# exprDotGenInvok
    |   expression '[' expression ']'									# exprArrayIndexing
    |   expression '(' expressionList? ')'								# exprMethodInvocation
    |   'new' creator													# exprNewCreator
    |   '(' typeType ')' expression										# exprCasting
    |   expression ('++' | '--')										# exprPostUnaryOp
    |   ('+'|'-'|'++'|'--') expression									# exprPreUnaryOp
    |   ('~'|'!') expression											# exprNegation
    |   expression ('*'|'/'|'%') expression								# exprMulDivMod
    |   expression ('+'|'-') expression									# exprAddSub
    |   expression ('<' '<' | '>' '>' '>' | '>' '>') expression			# exprBitShift
    |   expression ('<=' | '>=' | '>' | '<') expression					# exprComparison
    |   expression 'instanceof' typeType								# exprInstanceOf
    |   expression ('==' | '!=') expression								# exprEquality
    |   expression '&' expression										# exprBitAnd
    |   expression '^' expression										# exprBitXOR
    |   expression '|' expression										# exprBitOr
    |   expression '&&' expression										# exprLogicAnd
    |   expression '||' expression										# exprLogicOr
    |   expression '?' expression ':' expression						# exprConditional
